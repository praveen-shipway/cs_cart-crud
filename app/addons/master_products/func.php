...
function fn_master_products_filter_master_products_by_vendor(array $params)
{
    ...
    $join = db_quote(' LEFT JOIN ?:products AS master_products ON products.product_id = master_products.master_product_id');
    $condition = db_quote(' AND master_products.company_id IN (?n)', $vendor_ids);
 ...
 */
function fn_master_products_vendor_locations_get_products(array $params, array $filter_types, array $fields, array $sortings, &$condition, &$join)
{
    if (!$filter_types) {
        return;
    }
    ...
    $condition .= db_quote(' OR products.company_id = 0 AND master_products.company_id IN (?n)', $vendor_ids);
}
